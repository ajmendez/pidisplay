...
int main (void)
{
  ...
  for (;;)
  {
      ...
      sprintf(t, "   %d:%d:%d", localTime->tm_hour, localTime->tm_min, localTime->tm_sec);
      ...
      // currTime = time(NULL);
      // sprintf(t, "%s", ctime(&currTime));
      
      // char uptime[10];
      // unsigned long up = sys_info.uptime;
      // sprintf(ramInfo, "Uptime %ld sec", up);
